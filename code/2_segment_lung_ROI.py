...
def get_options():   
    parser = argparse.ArgumentParser(description='Step 2: segment lung ROI (region of interest)')
...
if __name__ == '__main__':
    options = get_options()
    output_path = options.working_dir + 'output_final/'
    ...
    for img_file in file_list:
        ...
        for i in range(len(imgs_to_process)):
            ...
            max_im = np.max(img)
            min_im = np.min(img)
            # To improve threshold finding, remove the underflow and overflow            
            img[img==max_im]=mean
            img[img==min_im]=mean
            # Separate foreground (radio-opaque tissue) and background (radio transparent tissue ie lungs)
            # This is done only in the center of image to avoid as many non-tissue parts as possible
            kmeans = KMeans(n_clusters=2).fit(np.reshape(middle,[np.prod(middle.shape),1]))
            centers = sorted(kmeans.cluster_centers_.flatten())
            threshold = np.mean(centers)
            thresh_img = np.where(img<threshold,1.0,0.0)  # threshold the image
            # Erosion: remove graininess from some regions
            # Dilation: make the lung region engulf the vessels and incursions into the lung cavity by radio opaque tissue
            ...
            good_labels = []
            for prop in regions:
                # The background region is removed by removing regions with a bbox that is too large in either dimension
                # Regions that are too close to the top and bottom are removed
                B = prop.bbox
    ...
    out_images = []      # set of images
    out_nodemasks = []   # set of nodemasks
    ...
    # generate random integer to separate train and test datasets
